...
pub fn key(a: *App, code: u32) bool {
    ...
    D.exportCsv(a) catch |e| {
        ...
        a.hud.toast(std.fmt.bufPrint(&buf, "export failed: {s}", .{@errorName(e)}) catch "export failed", 8);
        return true;
    };
    ...
    a.hud.toast("exported — CSV written in the working directory (console shows the file name)", 6);
    return true;
}
